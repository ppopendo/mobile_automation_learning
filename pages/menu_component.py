...
class MenuComponentLocators:
	...
	"""
	LOGIN_OPTION: Tuple[str, str] = field(
		default=(AppiumBy.ACCESSIBILITY_ID, "Login Menu Item"),
		init=False
	)
	LOGOUT_OPTION: Tuple[str, str] = field(
		default=(AppiumBy.XPATH, "//android.widget.TextView[@content-desc='Logout Menu Item']"),
		init=False
	)
	LOGOUT_POPUP_YES_BUTTON: Tuple[str, str] = field(
		default=(AppiumBy.ID, "android:id/button1"),
		init=False
	)
	LOGOUT_POPUP_NO_BUTTON: Tuple[str, str] = field(
		default=(AppiumBy.ID, "android:id/button2"),
		init=False
	)
...
class MenuComponent(BasePage):
	...
	def is_logout_option_displayed(self) -> bool:
		return self.is_element_displayed(MenuComponentLocators.LOGOUT_OPTION)
